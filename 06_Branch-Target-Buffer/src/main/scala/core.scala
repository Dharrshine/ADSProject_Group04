...
class PipelinedRV32Icore (BinaryFile: String) extends Module {
  ...
  //BTB Connections
  btb.io.PC := fetchStage.io.btbLookupPC

  btb.io.update := executeStage.io.outBtbUpdate
  btb.io.updatePC := executeStage.io.outBtbUpdatePC
  btb.io.updateTarget := executeStage.io.outBtbUpdateTarget
  btb.io.branchTaken := executeStage.io.outBtbBranchTaken
  btb.io.mispredicted := executeStage.io.outBtbMispredicted

  fetchStage.io.btbValid := btb.io.valid
  fetchStage.io.btbTarget := btb.io.target
  fetchStage.io.btbPredictTaken := btb.io.predictTaken

  IfBarrier.io.inBtbValid := btb.io.valid
  IfBarrier.io.inBtbPredictTaken := btb.io.predictTaken
  IfBarrier.io.inBtbTarget := btb.io.target

  IdBarrier.io.inBtbValid := IfBarrier.io.outBtbValid
  IdBarrier.io.inBtbPredictTaken := IfBarrier.io.outBtbPredictTaken
  IdBarrier.io.inBtbTarget := IfBarrier.io.outBtbTarget
